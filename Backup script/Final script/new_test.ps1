...
$url = "http://restprd.corp.netapp.com/asup-rest-interface/ASUP_DATA/client_id/Fujitsu_capacity/cluster_name/$cluster/asup_subject/*MANAGEMENT*/last/1"
        ...
        $url2 = "http://restprd.corp.netapp.com/asup-rest-interface/ASUP_DATA/client_id/smartsolve/sys_serial_no/$sys/asup_type/DOT-REGULAR"
		...
		$link = $xml.xml.results.system.asups.asup.asup_content_list
			...
			$link2 = $link[0]
			if($link2[0] -ne $null){
				$content2 = get_xml $link asup.xml $serial
				$links2 = $content2.xml.results.system.asups.asup.list.content
				$section2= @{}
				foreach($l2 in $links2){
					$name2=$l2.name
					$name2 = $name2.replace("-","")
					$link2=$l2.link
					$section2.ADD($name2, $link2)
				}
			}
...
[xml]$content2   = Get-Content -Path "C:\Temp\asup\AGGR-EFFICIENCY.XML"
$content2
