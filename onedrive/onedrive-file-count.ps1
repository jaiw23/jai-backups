...
$count_csv_file = "$dfs_path\$ticket-$date-$setting-$jobid.csv"
$mgmt_host = "mgmt_host"
...
$count_csv_file = $using:count_csv_file
...
$destination_csv = "$using:dfs_path\logs\$using:ticket-$using:region-$using:date-$using:setting-$using:jobid-file-count.csv"
$error_count = 0
$send_email = $true
...
foreach ($userpath in $content)
    {
    $path = $userpath
        log2file -log2console -file_name $log_file_name -message $("ID: " +$ID )
    try {
    if (Test-Path $path){
        $number = cmd /r dir /a:-d /s /b $path | find /c `"\\`"
        $out_csv = ''| Select-Object Share_Path,File_Count    
        $out_csv.Share_Path = $path
        $out_csv.File_Count = $number
        $output += $out_csv 
        log2file -log2console -file_name $log_file_name -message $("Completed count " + $using:setting + " for path " + $path + " File_count " + $number )
        }
        else {log2file -log2console -file_name $log_file_name -message $("ERROR: " + $path + " not reachable" )
              $error_count += 1
             }
        }
    catch{
        log2file -log2console -file_name $log_file_name -message "ERROR: Failed to apply ACL $Principal : $($path) $($Error[0])"
        $error_count += 1
    }
    }
    $output|export-csv -path $count_csv_file -NoTypeInformation
    ...
    try{
        ...
        Move-Item -Path $count_csv_file -Destination $destination_csv -Force
    }
    catch{
...
'
    ...
    try{
        ...
        $attachment2 = New-Object System.Net.Mail.Attachment –ArgumentList $destination_csv
