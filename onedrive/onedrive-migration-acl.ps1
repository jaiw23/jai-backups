...
$log_file_name_acl = "$dfs_path\$ticket-$date-$setting-$jobid-acl.txt"
$mgmt_host = "mgmt_host"
...
$log_file_name_acl = $using:log_file_name_acl
...
$destination_logs_acl = "$using:dfs_path\logs\acl\$using:ticket-$using:region-$using:date-$using:setting-$using:jobid-logs-acl.txt"
...
foreach ($userpath in $content)
    ...
    #commenting takeown step as per latest discussion with Nad and ODFB team
    <#
    try{
        takeown /a /r /d Y /f $path
    }
    catch{
        log2file -log2console -file_name $log_file_name -message "ERROR: Failed to take ownership for : $($path) $($Error[0])"
        $error_count += 1
    }#>
   
    try {
        log2file -log2console -file_name $log_file_name_acl -message $("ACL before applying migration ACL: " + $(Get-Acl $path | fl | out-string) )
        ...
        $Acl.AddAccessRule((New-Object System.Security.AccessControl.FileSystemAccessRule(
            $Principal,
            "$Permission",      # [System.Security.AccessControl.FileSystemRights]
        ...
        $Acl_after = Get-Acl $path
        log2file -log2console -file_name $log_file_name_acl -message $("ACL after applying migration ACL: " + $($Acl_after | fl | out-string) )
    ...
    try{
        ...
        Move-Item -Path $log_file_name_acl -Destination $destination_logs_acl -Force
    }
    catch{
