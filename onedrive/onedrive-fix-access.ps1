...
foreach ($userpath in $content)
    ...
    try {
         ...
         $map_drive_path_parent = Split-Path $path -Parent
         $map_drive_path_leaf = Split-Path $path -Leaf
         $map_output = net use Z: $map_drive_path_parent
         log2file -log2console -file_name $log_file_name -message $("Mapping Z: drive " + $map_output )
         $path_new = "Z:\$map_drive_path_leaf"
 
    if (Test-Path $path_new){
        $fix = icacls $path_new /inheritancelevel:d /t | Out-Null
        takeown /a /r /d Y /f $path_new
        log2file -log2console -file_name $log_file_name -message $("Completed setting " + $using:setting + " for path " + $path )
        }
        ...
        try {
             $unmap_output = & net use Z: /delete /y
             log2file -log2console -file_name $log_file_name -message $("Un-Mapping: " + $unmap_output )
        }
        catch{
              log2file -log2console -file_name $log_file_name -message "ERROR: Failed to unmap drive $path_new : $($Error[0])"
        }
    }
    try{
